...
def dancing_joshua(instate, outstate):
    if instate['count'] == 0:
        ...
        instate['joshua_window'] = np.zeros((60, 120, 4))  # HSVA format
        ...
        # Simplified wind parameters
        instate['wind'] = {
            'strength': 0,
            'target_strength': np.random.uniform(0.2, 0.4),
        ...
        # Main trunk
        trunk_base_x = 60 + np.random.randint(-15, 16)  # Centered with slight variation
        ...
        instate['trunk'] = {
            ...
            'color_variation': np.random.uniform(-0.02, 0.02)
        }
        ...
        right_angle_range = np.linspace(np.pi/6, np.pi/3, num_branches//2 + num_branches%2)
        left_angle_range = np.linspace(-np.pi/6, -np.pi/3, num_branches//2)
        
        # Add some noise to each angle to make it look more natural
        right_angle_range += np.random.uniform(-0.1, 0.1, size=len(right_angle_range))
        left_angle_range += np.random.uniform(-0.1, 0.1, size=len(left_angle_range))
        
        # Combine angles, alternating sides
        branch_angles = []
        for i in range(max(len(right_angle_range), len(left_angle_range))):
            if i < len(right_angle_range):
                branch_angles.append(right_angle_range[i])
            if i < len(left_angle_range):
                branch_angles.append(left_angle_range[i])
        ...
        height_positions = np.linspace(0.6, 1.0, num_branches)
        np.random.shuffle(height_positions)  # Shuffle to avoid obvious pattern
        ...
        for i in range(num_branches):
            ...
            # Branch length
            length = np.random.uniform(18, 25)
            ...
            sec_positions = np.linspace(0.5, 1.0, num_secondary)
            ...
            sec_angle_range = np.linspace(-np.pi/6, np.pi/6, num_secondary)
            # Add noise to angles
            sec_angle_range += np.random.uniform(-0.1, 0.1, size=num_secondary)
            
            # Shuffle the positions and angles for more natural look
            np.random.shuffle(sec_positions)
            np.random.shuffle(sec_angle_range)
            
            for j in range(num_secondary):
                # Position along primary branch (with better spacing)
                position = sec_positions[j]
                
                # Angle for this secondary branch (with better distribution)
                sec_angle = sec_angle_range[j]
                
                # Length and width
                sec_length = length * np.random.uniform(0.5, 0.7)
                sec_width = width * np.random.uniform(0.8, 0.95)
        ...
        instate['trunk_steps'] = 12  # Fewer steps than original but enough for quality
        instate['branch_steps'] = 10  # Fewer steps for branches
        instate['sec_branch_steps'] = 8  # Fewer steps for secondary branches
    ...
    trunk_sway = np.sin(trunk['sway_phase']) * wind['strength'] * 2.0
    ...
    y_coords = np.linspace(trunk_base_y, trunk_base_y - trunk_height, steps)
    ...
    for branch in instate['branches']:
        ...
        branch_sway = np.sin(branch['sway_phase']) * wind['strength'] * 2.0
        branch_angle = branch['angle'] + branch_sway * 0.2
        
        # Calculate branch end position
        end_x = branch_x + np.cos(branch_angle) * branch['length']
        end_y = branch_y + np.sin(branch_angle) * branch['length']
        ...
        for i in range(steps):
            ...
            bend = np.sin(t * np.pi) * 0.3
            local_angle = branch_angle + bend
            
            # Calculate position along branch
            pos_x = branch_x + np.cos(local_angle) * branch['length'] * t
            pos_y = branch_y + np.sin(local_angle) * branch['length'] * t
        ...
        for sec_branch in branch['secondary_branches']:
            ...
            sec_base_x = branch_x + np.cos(branch_angle) * branch['length'] * sec_pos
            sec_base_y = branch_y + np.sin(branch_angle) * branch['length'] * sec_pos
            
            # Calculate secondary branch angle, combining primary angle and its own
            sec_angle = branch_angle + sec_branch['angle']
            
            # Add secondary sway
            sec_sway = np.sin(sec_branch['sway_phase']) * wind['strength'] * 1.5
            sec_angle += sec_sway * 0.1
            
            # Calculate secondary branch end position
            sec_end_x = sec_base_x + np.cos(sec_angle) * sec_branch['length']
            sec_end_y = sec_base_y + np.sin(sec_angle) * sec_branch['length']
            ...
            for i in range(steps):
                ...
                # Add waviness
                bend = np.sin(t * np.pi) * 0.4
                local_angle = sec_angle + bend
                
                # Position along secondary branch
                pos_x = sec_base_x + np.cos(local_angle) * sec_branch['length'] * t
                pos_y = sec_base_y + np.sin(local_angle) * sec_branch['length'] * t
...
# Optimized leaf cluster drawing function
def draw_leaf_cluster_optimized(window, x_indices, y_indices, center_x, center_y, size, wind_strength, sway_phase, color_variation, fade_factor):
    # Skip if out of bounds
    if center_x < 0 or center_x >= 120 or center_y < 0 or center_y >= 60:
        return
    
    # Apply leaf cluster sway
    cluster_sway = np.sin(sway_phase) * wind_strength * 1.5
    ...
    angle_noise = np.random.uniform(-0.2, 0.2, size=num_spikes)
    spike_angles = base_angles + angle_noise
    
    for i, actual_angle in enumerate(spike_angles):
        spike_length = size * np.random.uniform(0.8, 1.4)
        
        # Simplified drawing - use fewer steps
        ...
        for step in range(spike_steps):
            ...
            spike_x = center_x + np.cos(actual_angle) * d
            spike_y = center_y + np.sin(actual_angle) * d
